#! /Library/Frameworks/Python.framework/Versions/3.7/bin/python3 -u
...
def read_gmm(fileGMM1):
    ...
    try:
        with open(fileGMM1, 'rb') as fpGmm:
            headIn = fpGmm.read(15)

            if headIn != header:
                print(f'ERROR: {fileGMM1} is not a valid GMM file')
                exit(-1)
        ...
        raise Exception(f'Error al leer el fichero {fileGMM1}')


def read_gmm(fileGMM2):
    '''
       Reads the weights, means and convariances from a GMM
       stored using format "UPC: GMM V 2.0"
    '''
    ...
    try:
        with open(fileGMM2, 'rb') as fpGmm:
            headIn = fpGmm.read(15)

            if headIn != header:
                print(f'ERROR: {fileGMM2} is not a valid GMM file')
                exit(-1)
        ...
        raise Exception(f'Error al leer el fichero {fileGMM2}')
...
def plotGMM(fileGMM1, fileGMM2, xDim, yDim, percents, colorGmm, filesFeat1=None, filesFeat2=None, colorFeat=None, limits=None, subplot=[221,222,223,224]):
    for i in range(0,4):
        if i is 0:
            fileGMM = fileGMM1
            filesFeat = fileFeat1
...
if __name__ == '__main__':
    ...
    plotGMM(fileGMM1, xDim, yDim, percents, 'r', filesFeat, 'r', limits, 221)
    plotGMM(fileGMM1, xDim, yDim, percents, 'r', filesFeat, 'b', limits, 222)
    plotGMM(fileGMM2, xDim, yDim, percents, 'b', filesFeat, 'r', limits, 223)
    plotGMM(fileGMM2, xDim, yDim, percents, 'b', filesFeat, 'b', limits, 224)
    plt.show()
